...
//
// ## 현장 조정 패드 (2026-09-15 — 팀원1 위치·크기 맞추기용)
// 에셋이 뜨면 오른쪽에 「조정」 버튼이 보이고, 패드로 **이동(화면 기준)·회전·크기**를 바꾼다.
// 값은 앵커 로컬 오프셋으로 바꿔 **서버 앵커 행에 자동 저장**(마지막 탭 1.5초 뒤 PUT .../asset-offset)
// → 재설치·다른 폰·리빌드와 무관하게 다음 스캔부터 그 자리에 뜬다.
// 합성: 회전 = 앵커 회전 + (ini yaw + 저장 yaw) · 위치 = 앵커 + 앵커회전·저장오프셋 + (0,0,ini z)
//       · 배율 = ini 배율 × 저장 배율. 회전은 **제자리**(오프셋 위치를 축으로) 돈다.
// 앵커를 새로 등록(cloud_id 변경)하면 서버가 오프셋을 지운다 — 옛 앵커 기준값이라서.
//
// ## 빌드 분리
...
struct FNavAssetAnchorEntry
{
	...
	/** 현재 적용 중인 배치 보정(앵커 로컬 cm · yaw ° · 배율). 조정 패드가 바꾼다. */
	UPROPERTY() FVector OffLoc = FVector::ZeroVector;
	UPROPERTY() float OffYaw = 0.f;
	UPROPERTY() float OffScale = 1.f;
	/** 서버에 마지막으로 저장(또는 서버에서 받은) 값 — 「저장값으로」 되돌리기용. */
	UPROPERTY() FVector SavedLoc = FVector::ZeroVector;
	UPROPERTY() float SavedYaw = 0.f;
	UPROPERTY() float SavedScale = 1.f;
};
...
private:
	...
	// ── 조정 패드 ─────────────────────────────────────────────────────────
	/** 에셋이 뜨면 패드 위젯을 만든다(한 번). */
	void EnsureTuneOverlay();
	void HandleTune(ENavAssetTune Code);
	/** 패드 본문(대상·값·저장 상태)과 토글 버튼 표시 여부를 갱신한다. */
	void RefreshTuneUi();
	/** 지금 조정 대상(떠 있는 에셋 중 하나). 없으면 nullptr. */
	FNavAssetAnchorEntry* TuneTarget();
	/** 디바운스 만료된 변경을 서버에 PUT 한다(틱마다). bForce 면 기다리지 않는다. */
	void FlushOffsetSaves(bool bForce);
	void PutOffset(const FNavAssetAnchorEntry& Entry);
	/** 앵커 pose + ini + 저장 보정 → 월드 트랜스폼. */
	FTransform ComposeTarget(const FNavAssetAnchorEntry& Entry, const FTransform& AnchorXf) const;

	UPROPERTY(Transient)
	TObjectPtr<UNavAssetTuneOverlay> TuneOverlay = nullptr;
	int32 TunePointNo = INDEX_NONE;
	int32 TuneStepLevel = 1;                 // 0=미세 1=보통 2=크게
	TSet<int32> DirtyOffsetPoints;           // 저장 대기
	double OffsetSaveDueTime = 0.0;          // 이 시각 이후 저장
	int32 OffsetSavesInFlight = 0;
	FString TuneSaveState;                   // 패드 하단 상태 한 줄
	FColor TuneSaveColor = FColor::White;
	double ResetArmedUntil = 0.0;            // 초기화는 2초 안에 두 번 눌러야 한다
